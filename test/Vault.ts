...
import { EVMVaultFactory, TestProtocol, TestProtocolFactory } from "../types/evm";
...
describe("VaultFactory EVM", () => {
    ...
    let protocol: TestProtocolFactory;
    let protocolHelper: TestProtocol;
    ...
    before(async () => {
        ...
        await token.mint(vaultCreator.address, ethers.parseUnits("1000000", await token.decimals()));
        await token.mint(user1.address, ethers.parseUnits("1000000", await token.decimals()));
        await token.mint(user2.address, ethers.parseUnits("1000000", await token.decimals()));

        const TestProtocolFactoryFactory = await ethers.getContractFactory("TestProtocolFactory");
        protocol = await TestProtocolFactoryFactory.deploy(await token.getAddress());

        const TestProtocolFactory = await ethers.getContractFactory("TestProtocol");
        protocolHelper = await upgrades.deployProxy(TestProtocolFactory, [deployer.address]);
        await protocolHelper.addPool(await token.getAddress(), await protocol.getAddress());
    });

    it("should create a new vault", async () => {
        //Approve the vault factory to spend the token
        await token.connect(vaultCreator).approve(await vaultFactory.getAddress(), ethers.parseUnits("1000", await token.decimals()));
        ...
        const vaultFactoryValue = {
            ...
            deadline: deadline,
            protocolHelper: await protocolHelper.getAddress(),
            name: "Test Vault",
            symbol: "TV",
        ...
        const vaultFactorySignature = await signer.signTypedData(vaultFactorydomain, vaultFactoryTypes, vaultFactoryValue);

        const createNewVaultTx = await vaultFactory.connect(vaultCreator).createNewVault({
            authority: vaultFactoryValue.authority,
            protocolHelper: vaultFactoryValue.protocolHelper,
        ...
        assert(vaultCreatorShares > 0, "Vault creator shares should be greater than 0");
        ...
        await token.connect(user1).approve(vaultAddress, ethers.parseUnits("1000", await token.decimals()));
        await token.connect(user2).approve(vaultAddress, ethers.parseUnits("1000", await token.decimals()));
        ...
        const vaultSignature1 = await signer.signTypedData(vaultDomain, vaultDepositTypes, depositValue1);
        await vault.connect(user1).deposit(depositValue1.depositId, depositValue1.amount, depositValue1.user, depositValue1.vaultTvl, depositValue1.deadline, vaultSignature1);
        ...
        try {
            ...
            const vaultSignature2 = await signer.signTypedData(vaultDomain, vaultDepositTypes, depositValue2);
            await vault.connect(user2).deposit(depositValue2.depositId, depositValue2.amount, depositValue2.user, depositValue2.vaultTvl, depositValue2.deadline, vaultSignature2);
        } catch (error) {
            assert(error instanceof Error, "Error should be an instance of Error");
            assert(error.message.includes("Invalid deposit amount"), "Error message should include 'Invalid deposit amount'");
        }
        ...
        const vaultSignature3 = await signer.signTypedData(vaultDomain, vaultDepositTypes, depositValue3);
        await vault.connect(user2).deposit(depositValue3.depositId, depositValue3.amount, depositValue3.user, depositValue3.vaultTvl, depositValue3.deadline, vaultSignature3);
        ...
        await token.mint(await vault.getAddress(), ethers.parseUnits("1000", await token.decimals()));
        ...
        const vaultSignature = await signer.signTypedData(vaultDomain, vaultWithdrawTypes, withdrawValue);

        const withdrawTx = await vault.connect(user1).withdraw(withdrawValue.withdrawId, withdrawValue.user, withdrawValue.amountOut, withdrawValue.vaultTvl, withdrawValue.vaultFee, withdrawValue.creatorFee, withdrawValue.deadline, vaultSignature);
